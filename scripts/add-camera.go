...
func main() {
	...
	if err != nil {
		log.Fatalf("Failed to upsert camera: %v", err)
	}
	...
	_, err = db.ExecContext(ctx, `
		...
		ON CONFLICT (tenant_id, camera_id, profile_token) 
		DO UPDATE SET rtsp_url_sanitized = EXCLUDED.rtsp_url_sanitized, video_codec = EXCLUDED.video_codec`,
		profileID, tenantID, cameraID, "main", "Main Stream",
		"H264", 1920, 1080, 25, 4096, rtspURL)
	if err != nil {
		log.Fatalf("Failed to upsert media profile: %v", err)
	}
	...
	_, err = db.ExecContext(ctx, `
		...
		DO UPDATE SET main_rtsp_url_sanitized = EXCLUDED.main_rtsp_url_sanitized, sub_rtsp_url_sanitized = EXCLUDED.sub_rtsp_url_sanitized`,
		tenantID, cameraID,
		"main", rtspURL, true,
		"main", rtspURL, true, true)
	if err != nil {
		log.Fatalf("Failed to upsert stream selection: %v", err)
	}
